...
//cprintf("fault_va : %p\n",fault_va);
//print_page_working_set_or_LRUlists(curenv) ;
	struct WorkingSetElement *elementToFind;
	struct WorkingSetElement *ele;
	int found = 0;
	LIST_FOREACH(ele, &(curenv->SecondList))
	{
		if (ele->virtual_address == fault_va) {
			elementToFind = ele;
			found = 1;
		}
	}
	...
	if (found == 1) {
		if (LIST_SIZE(&(curenv->ActiveList)) == curenv->ActiveListSize) {
			struct WorkingSetElement* element2 = LIST_LAST(&curenv->ActiveList);
			LIST_REMOVE(&curenv->SecondList, elementToFind);
			LIST_REMOVE(&curenv->ActiveList, element2);
			LIST_INSERT_HEAD(&curenv->SecondList, element2);
			LIST_INSERT_HEAD(&curenv->ActiveList, elementToFind);
			pt_set_page_permissions(curenv, element2->virtual_address, 0,
					PERM_PRESENT);
			pt_set_page_permissions(curenv, elementToFind->virtual_address,
					PERM_PRESENT, 0);
		} else {
			LIST_REMOVE(&curenv->SecondList, elementToFind);
			LIST_INSERT_HEAD(&curenv->ActiveList, elementToFind);
		...
		if (LIST_SIZE(&(curenv->ActiveList)) == curenv->ActiveListSize
				&& curenv->SecondListSize == 0) {
			//cprintf("cant add more \n");

			struct WorkingSetElement * elementToremove = LIST_LAST(
					&curenv->ActiveList);
			LIST_REMOVE(&curenv->ActiveList, elementToremove);
			pt_set_page_permissions(curenv, elementToremove->virtual_address, 0,
					PERM_PRESENT);

			uint32 page_permissions = pt_get_page_permissions(curenv,
					elementToremove->virtual_address);
			struct Frame_Info *ptr_frame_info;
			if (page_permissions & PERM_MODIFIED) {
				uint32 *ptr_page_table;
				ptr_frame_info = get_frame_info(curenv->env_page_directory,
						(void *) elementToremove->virtual_address,
						&ptr_page_table);
			...
			unmap_frame(curenv->env_page_directory,
					(void*) elementToremove->virtual_address);
			LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), elementToremove);

			struct Frame_Info *ptr_Finfo;
			allocate_frame(&ptr_Finfo);
			map_frame(curenv->env_page_directory, ptr_Finfo, (void*) fault_va,
					PERM_WRITEABLE | PERM_USER);
			int ret = pf_read_env_page(curenv, (void*) fault_va);
			if (ret == E_PAGE_NOT_EXIST_IN_PF) {
				if (fault_va < USTACKTOP && fault_va >= USTACKBOTTOM) {
					int ret2 = pf_add_empty_env_page(curenv, fault_va, 0);
				} else {
					panic("invalid address");
				}
			}
			struct WorkingSetElement *faulted_page = LIST_LAST(
					&curenv->PageWorkingSetList);
			faulted_page->virtual_address = fault_va;
			pt_set_page_permissions(curenv, fault_va, PERM_PRESENT, 0);
			LIST_REMOVE(&curenv->PageWorkingSetList, faulted_page);
			...
			if (LIST_SIZE(&(curenv->ActiveList)) == curenv->ActiveListSize) {
				...
				if (LIST_SIZE(&(curenv->SecondList))
					...
					//cprintf("we are in\n");
					struct WorkingSetElement* element = LIST_LAST(
							&curenv->SecondList);
					uint32 page_permissions = pt_get_page_permissions(curenv,
					...
					unmap_frame(curenv->env_page_directory,
							(void*) element->virtual_address);
					LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), element);
				}
				LIST_INSERT_HEAD(&curenv->SecondList, elementToremove);
				pt_set_page_permissions(curenv,
						elementToremove->virtual_address, 0, PERM_PRESENT);
			}
			struct Frame_Info *ptr_Finfo;
			allocate_frame(&ptr_Finfo);
			map_frame(curenv->env_page_directory, ptr_Finfo, (void*) fault_va,
