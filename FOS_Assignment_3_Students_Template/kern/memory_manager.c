...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	int allocated_pages = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	uint32 va_address = virtual_address;
	numbers = ROUNDUP(size,PAGE_SIZE) / PAGE_SIZE;
	for (int i = 0; i < numbers; i++) {
		pf_remove_env_page(e, va_address);
		va_address += PAGE_SIZE;
	}
	//2. Free ONLY pages that are resident in the working set from the memory
	struct WorkingSetElement* object;
	va_address = virtual_address;
	for (int i = 0; i < numbers; i++, va_address += PAGE_SIZE) {
		LIST_FOREACH(object,&e->ActiveList)
		{
			if (object->virtual_address == va_address) {
				LIST_REMOVE(&e->ActiveList, object);
				LIST_INSERT_HEAD(&e->PageWorkingSetList, object);
				unmap_frame(e->env_page_directory, (uint32*) va_address);
				if (LIST_SIZE(&e->SecondList) != 0) {
					object = LIST_FIRST(&e->SecondList);
					pt_set_page_permissions(e, object->virtual_address,
					PERM_PRESENT, 0);
					LIST_REMOVE(&e->SecondList, object);
					LIST_INSERT_TAIL(&e->ActiveList, object);
				}
				break;
			}
		}
		LIST_FOREACH(object,&e->SecondList)
		{
			if (object->virtual_address == va_address) {
				LIST_REMOVE(&e->ActiveList, object);
				LIST_INSERT_HEAD(&e->PageWorkingSetList, object);
	...
	for (int i = 0; i < numbers; i++) {
		k = 0;
		get_page_table(e->env_page_directory, (uint32*) va_address, &p_p_t);
		if (p_p_t != NULL) {
			for (int j = 0; j < 1024; j++) {
				if (p_p_t[j] != 0 && PERM_PRESENT != 0) {
					k = -1;
					break;
				}
			}
			if (k == 0) {
				fInfo = to_frame_info(
						((e->env_page_directory[PDX(va_address)]) >> 12)
								* PAGE_SIZE);
				fInfo->references = 0;
				free_frame(fInfo);
				e->env_page_directory[PDX(va_address)] = 0;

			}
		}
		va_address += PAGE_SIZE * 1024;
	}
}
...
void env_page_ws_print(struct Env *curenv) {
	...
	for (i = 0; i < (curenv->page_WS_max_size); i++) {
		...
		cprintf(", used= %d, modified= %d, buffered= %d", isUsed, isModified,
				isBuffered);
